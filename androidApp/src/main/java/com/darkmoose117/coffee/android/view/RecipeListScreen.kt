...
fun RecipeListScreen(
    ...
    when (state) {
        RecipeListState.Loading -> Box(modifier) {
            Text(text = "Loading...")
        }
        is RecipeListState.Error -> Box(modifier) {
...
@Composable
fun RecipeRow(modifier: Modifier = Modifier, text: String, onClick: (() -> Unit)? = null) = Box(
    modifier = modifier
        .fillMaxWidth()
...
@Preview
@Composable
fun RecipeListScreenPreview() = ThemedPreview {
    RecipeListScreen(
        state = RecipeListState.Loaded(
            listOf(
